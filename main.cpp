...
std::array<std::array<int, 25 /* num. skills */>, 18 /* num. playstyles */> playstyleMultipliers = {{
	// blank
			...
			switch(LOWORD(W))
			{
				...
				case IDB_MAKE_SILV:
				{
					if(HIWORD(W)==BN_CLICKED)
					{
						...
						for (int ii = IDT_ABIL_ATKP; ii < gi_lastAbility; ii += 2)
						{
							_itow_s(80 + values[(ii - IDT_ABIL_ATKP) / 2], buffer, 3, 10);
							SendDlgItemMessage(ghw_tab1, ii, WM_SETTEXT, 0, (LPARAM)buffer);
						}
				...
				case IDB_MAKE_REGU:
				{
					if(HIWORD(W)==BN_CLICKED)
					{
						...
						for (int ii = IDT_ABIL_ATKP; ii < gi_lastAbility; ii += 2)
						{
							_itow_s(70 + values[(ii - IDT_ABIL_ATKP) / 2], buffer, 3, 10);
							SendDlgItemMessage(ghw_tab1, ii, WM_SETTEXT, 0, (LPARAM)buffer);
						}
